...
public class PersonnelManagePeoplePage extends JFrame {
    ...
    private void SearchButActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        if (x1.length() > 0 && x2.length() > 0) {
            ...
            Main.userType = 1;          //Used to denote the type of user being searched (Current Index: 1 = People, 2 = Personnel).  
            DataIO.checkUser(x1, x2);
            //userToSearch is found and set!
            DefaultTableModel peoModelA = (DefaultTableModel) jTable1.getModel();
            peoModelA.setRowCount(0);
            Object rowData[] = new Object[9];
            for (int i = 0; i < DataIO.allUserSearch.size(); i++) {
                ...
                rowData[7] = DataIO.allUserSearch.get(i).getCitizenStat();
