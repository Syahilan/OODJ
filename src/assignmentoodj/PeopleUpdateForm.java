...
public class PeopleUpdateForm extends JFrame {
    ...
    public PeopleUpdateForm() {
        Main.peoToUpdate = Main.loginPeo;
        initComponents();
        initFields();
    }
    ...
    private void BackButActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        PeopleMainPage pmpDX = new PeopleMainPage();
        pmpDX.setVisible(true);
    }
            ...
            if (userExisted != null) {
                ...
                if (x1.length() > 0 && x2.length() > 0 && x3.length() > 0 && x4.length() > 0 && x5.length() > 0 && x6.trim().length() > 0 && x7.length() > 0 && x8.length() > 0 && x9.length() > 0 && x10.length() > 0 && x11.length() > 0 && x12.length() > 0) {
                    if (x5age >= 18) {
                        ...
                        this.setVisible(false);
                        PeopleMainPage pmpE = new PeopleMainPage();
                        pmpE.setVisible(true);
                        dispose();
                    } else {
